...
- ✅ **6.3 Conductor pi/py loud-failure on missing credentials.** Pi/py modes were silently failing when API keys were absent (pi prints a 401 to stderr but exits with code 0; daemon reads `pane_closed/done`; brain-mcp session sits in `queued` forever). Conductor now resolves the provider via `resolvePiModelSpec`, checks `PROVIDER_ENV_KEYS` for the matching env var, and if none are set: cleans up the tmpdir, marks the session `failed` with a clear error including the expected env var names and a nudge toward `mode="pi-core"`, and skips the spawn. Same check added for `py` mode (always anthropic). `claude` and other-CLI paths unchanged — they handle their own auth flows.
- ✅ **6.4 Burn-in observability tools.**
  - `brain_metrics view=daemon_summary` exposes `db.getDaemonSummary` over MCP — total spawns + success rate by transport (`tmux-bash` | `tmux-daemon` | `headless`). Use this during burn-in to confirm the daemon path is being exercised vs. legacy bash.
  - New tool **`brain_daemon_status`**: at-a-glance health for the agent-watcher daemon — current mode, daemon pid + alive check (via `process.kill(pid, 0)`), pane_watches counts (active + last-1h terminal breakdown), open detached `brain-XXX` tmux sessions, effective default timeout. Replaces the manual `pgrep -af agent-watcher.js && sqlite3 …` invocation.
  - DB additions to support both: `daemonLock_record`, `paneWatch_statusCounts`, `paneWatch_activeDetachedSessions`. No schema change; just new query helpers.
